...
headers.append('referer', 'https://m.y.qq.com/')


class QQMusic extends Engine {
  search({ keyword, curPage }: EngineSearchParam): Promise<{ list: Song[], total: number, curPage: number }> {
    ...
    return fetch(target_url, data, 'get', headers)
      .then((response: any) => {
        let { data: { song } } = response;
        const list = song.list.map(item => qqConvertSong(item));
        return {
          list,
          total:song.totalnum,
          curPage
...
function qqConvertSong(song: any):Song {
  const d = {
    id: song.songmid,
    name: song.songname,
    singer: song.singer.map(({ name, mid, id }: any) => { return { name, mid, id } }),
    album: song.albumname,
    albumid: song.albummid,
    source: 'qq',
    sourceurl: `http://y.qq.com/#type=song&mid=${song.songmid}&tpl=yqq_song_detail`,
    canPlay: song.pay.payplay !== 0,
  };
  return d;
}
