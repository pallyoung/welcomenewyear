package host.spencer.views.switchView;
...
public class Switch extends View {
    ...
    private int onTintColor = 0xff4bd763;
    private int thumbTintColor = 0xffffffff;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        paint.setColor(isOn ? onTintColor:0xffe3e3e3);
        canvas.drawPath(sPath, paint);

        sAnim = sAnim - 0.02f > 0 ? sAnim - 0.02f : 0;
        bAnim = bAnim - 0.02f > 0 ? bAnim - 0.02f : 0;
        ...
        paint.setStyle(Paint.Style.FILL);
        paint.setColor(thumbTintColor);
        canvas.drawPath(bPath, paint);

        paint.setStyle(Paint.Style.STROKE);
        paint.setStrokeWidth(bStrokeWidth * 0.5f);

        paint.setColor(isOn ? onTintColor : 0xffbfbfbf);
        canvas.drawPath(bPath, paint);
    ...
    private void refreshState(int newState) {
        if (!value && newState == STATE_SWITCH_ON) {
            value = true;
        } else if (value && newState == STATE_SWITCH_OFF) {
            value = false;
        }
        lastState = state;
        state = newState;
    ...
    public void setValue(boolean value) {
        refreshState(value? STATE_SWITCH_ON : STATE_SWITCH_OFF);
    }
    public void setTintColor(String colorString){
        tintColor = Color.parseColor(colorString);
    }
    public void setOnTintColor(String colorString){
        onTintColor = Color.parseColor(colorString);
    }
    public void setThumbTintColor(String colorString){
        thumbTintColor = Color.parseColor(colorString);
    }
     ...
     * @param value
     */
    public void toggleSwitch(final boolean value) {
        this.value = value;
        postDelayed(new Runnable() {
            @Override
            public void run() {
                toggleSwitch(value ? STATE_SWITCH_ON : STATE_SWITCH_OFF);
            }
        }, 300);
    }
    ...
    public void onRestoreInstanceState(Parcelable state) {
        ...
        this.value = ss.value;
        this.state = this.value ? STATE_SWITCH_ON : STATE_SWITCH_OFF;
    }
